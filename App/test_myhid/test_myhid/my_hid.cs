...
namespace MyHID
{
    class HID
    {
        ...
        public HID_DEVICE[] pDevice = null ;
        public HID_DEVICE HidDevice;
        int Index = 0;
        public int nbrDevice = 0;
        public Byte[] outReport = new byte[64];
        public Byte[] inReport = new byte[64];
        ...
        /*
         * return : index of the found device
         * 
         */
        public int detect_device(UInt32 vid, UInt32 pid)
        {
         ...
         */
        public UInt32 sendReport()
        {
            UInt32 byte_count = 0;
            if (isFound() == false)
                return byte_count;
            byte[] buffer = new byte[HidDevice.Caps.OutputReportByteLength];
            /* Copy outReport to buffer */
            Array.Copy(outReport, 0, buffer, 1, 64);
            bool success = WriteFile(HidDevice.HidDevice, buffer, HidDevice.Caps.OutputReportByteLength, ref byte_count, IntPtr.Zero);
            return byte_count;
        }
         ...
         */
        public UInt32 readReport()
        {
            UInt32 byte_count = 0;
            if (isFound() == false)
                return byte_count;

            byte[] buffer = new byte[HidDevice.Caps.InputReportByteLength];
            bool success = ReadFile(HidDevice.HidDevice, buffer, HidDevice.Caps.InputReportByteLength, ref byte_count, IntPtr.Zero);
            /* Copy 64 bytes from buffer to inReport */
            Array.Copy(buffer, 1, inReport, 0, 64);
            return byte_count;
        }
