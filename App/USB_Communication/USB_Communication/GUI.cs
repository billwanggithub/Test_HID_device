...
namespace USB_Communication
{
    public partial class GUI : Form
    {
        ...
        {
            textBox_VID.Text = "0483";
            textBox_VIDa.Text = "0483";
            textBox_PID.Text = "5750";
            textBox_PIDa.Text = "5750";
            textBox_UsagePage.Text = "1";
            textBox_Usage.Text = "0";
            textBox_RID.Text = "1";
        }
        ...
        {
            read_parameters();
            Thread SendThread = new Thread(new ThreadStart(test_hid));
            SendThread.IsBackground = true;
            SendThread.Start();

        }

        public void test_hid()
        {
            int test_count = 0;
            while (true)
            {
        ...
        public void test_hid_rw()
        {
            int index = detect_device(DEVICE_VID, DEVICE_PID);
            if (index >= nbrDevice)
            {
            ...
            HID_DEVICE HidDevice = pDevice[index];

            outReport = new Byte[HidDevice.Caps.OutputReportByteLength];
            for (int i = 0; i < HidDevice.Caps.OutputReportByteLength; i++)
            {
                outReport[i] = (byte)(i * 3 + 1);
            }
            UInt32 byte_count = 0;
            WriteFile(HidDevice.HidDevice, outReport, HidDevice.Caps.OutputReportByteLength, ref byte_count, IntPtr.Zero);
            Console.WriteLine($"Write {byte_count} bytes");
            //Console.WriteLine(System.Text.Encoding.ASCII.GetString(outReport, 0, HidDevice.Caps.OutputReportByteLength));
            Console.WriteLine(BitConverter.ToString(outReport));

            byte_count = 0;
            inReport = new Byte[HidDevice.Caps.InputReportByteLength];
            bool success = ReadFile(HidDevice.HidDevice, inReport, HidDevice.Caps.InputReportByteLength, ref byte_count, IntPtr.Zero);
            Console.WriteLine($"Read {byte_count} bytes");
            //Console.WriteLine(System.Text.Encoding.ASCII.GetString(inReport, 0, HidDevice.Caps.InputReportByteLength));
            Console.WriteLine(BitConverter.ToString(inReport));
        }

        public int detect_device(int vid, int pid)
        {
            nbrDevice = FindNumberDevices();
            Console.WriteLine($"found device number = {nbrDevice }");
            ...
            {
                var Count = 0;
                if (pDevice[Index].Attributes.VendorID == vid && vid != 0)
                {
                    Count++;
                }
                if (pDevice[Index].Attributes.ProductID == pid)
                {
                    Count++;
                }
        ...
        void    OpenHidDevice(string DevicePath, ref HID_DEVICE[] HidDevice, int iHIDD)
        {
            ...
            if (iHIDD > (HidDevice.Length -1)) // 210708: modified by bill
                return;
...
//            WriteData = byte.Parse(textBox_Send.Text);
        ...
        {
            //Thread ReadThread = new Thread(new ThreadStart(HIDRead));
            //ReadThread.IsBackground = true;
            //ReadThread.Start();
        }
        private void button_Send_Click(object sender, EventArgs e)
        {
            //Thread SendThread = new Thread(new ThreadStart(HIDSend));
            //SendThread.IsBackground = true;
            //SendThread.Start();
        }
